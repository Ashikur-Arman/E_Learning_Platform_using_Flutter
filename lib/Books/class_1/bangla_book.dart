...
class BanglaBook extends StatefulWidget {
  @override
  _BanglaBookState createState() => _BanglaBookState();
}
...
class _BanglaBookState extends State<BanglaBook> {
  ...
  Future<void> loadPDF() async {
    try {
      ...
      final ByteData bytes = await rootBundle.load("assets/pdf/class_one_bangla.pdf");
      final Uint8List list = bytes.buffer.asUint8List();

      // টেম্প ফোল্ডারে PDF সংরক্ষণ করা
      final tempDir = await getTemporaryDirectory();
      final file = await File("${tempDir.path}/class_one_bangla.pdf").create();
